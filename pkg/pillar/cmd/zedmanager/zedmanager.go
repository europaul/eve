...
func quantifyChanges(config types.AppInstanceConfig, oldConfig types.AppInstanceConfig,
	...
	if len(oldConfig.AppNetAdapterList) != len(config.AppNetAdapterList) {
		...
		// neither needPurge nor needRestart. The new AppNetworkConfig is not
		// handed to zedrouter either while the guest runs (see
		// MaybeAddAppNetworkConfig), so the network is not reconfigured
		// underneath it; the config is published on the next restart, when the
		// domain is re-created with the new set of VIFs.
		...
		// (not a purge), is not auto-applied, and is withheld from zedrouter
		// until the application is restarted. ACL changes are applied live by
