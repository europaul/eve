...
func writelogFile(logChan <-chan inputEntry, moveChan chan fileChanInfo) {
	...
	uploadSockWriter := NewBufferedSockWriter(uploadSockVectorSource, 1000, 5*time.Second)
	keepSockWriter := NewBufferedSockWriter(keepSockVectorSource, 1000, 5*time.Second)
